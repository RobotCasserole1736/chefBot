...
import interpreter.Method.Type;
...
import frc.robot.RobotHardwareInputs;
import frc.robot.RobotHardwareOutputs;
...
public class Kitchen {
	...
	public Container cook() throws ChefException {
		...
		methodloop: while (i < methods.size() && !deepfrozen) {
			...
			switch (m.type) {
				case Take : 
					double inVal = RobotHardwareInputs.getInstance().getInput(m.fridgeIdx);
					ingredients.get(m.ingredient).setAmount(inVal);
					break;
				case Put :
				...
				case Stir :
					...
					break;
				case StirInto :
					long stirTime = Math.round(ingredients.get(m.ingredient).getAmount());
					if(stirTime > Integer.MAX_VALUE){
						throw new ChefException(ChefException.METHOD, m.n, m.type.toString(), "You made the chefs to Stir stuff into other stuff too much, so they all quit their jobs and went home. Oops.");
					}
					mixingbowls[m.mixingbowl].stir((int)stirTime);
					break;
				case Mix :
	...
	private void serve(int n) {
		for (int i = 0; i < n && i < bakingdishes.length; i++) {
			if(i < RobotHardwareOutputs.getInstance().getNumOutputs()){
				// The first N baking dishes get the top of the stack sent to actual hardware
				String resultStr = bakingdishes[i].serve();
				String[] results = resultStr.split(" ");
				String mostRecentResult = results[results.length - 1];
				if(mostRecentResult.length() > 0){
					RobotHardwareOutputs.getInstance().setOutput(i, Double.parseDouble(mostRecentResult));
				}
			} else {
				//Other baking dishes get printed
